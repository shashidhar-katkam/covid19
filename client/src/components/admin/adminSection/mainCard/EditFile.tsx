...
import { IFile, IFileC } from "../../../../models/models";
...
class EditImages extends React.Component<IProps, IState> {
    ...
    componentDidMount() {
        ...
        this.service.getImagesM({}).then((res: any) => {
            console.log(res);
            if (res.status) {
                this.setState({
                    AllInfo: res.data,
    ...
    _onScrollEvent() {
        ...
        if (d.scrollTop > totalHeight - 1300 && !this.state.isLoadNext) {
            if (!this.state.isLoadNext) {
                ...
                this.service.getImagesM({ skip: this.state.skip }).then((res: any) => {
                    if (res.status) {
    ...
    private _removeFile(id: string, currentStatus: boolean) {
        ...
        this.service.updateImageM({ _id: id, show: !currentStatus }).then((res: any) => {
            //console.log(res);
            if (res.status) {
                        ...
                        <>
                            {this.state.AllInfo.map((image: IImages) => {
                                return <div className="img-div">
                                    <img className="sp-img" src={`http://localhost:7777${image.files[0].filePath}`} alt="cardd" />
